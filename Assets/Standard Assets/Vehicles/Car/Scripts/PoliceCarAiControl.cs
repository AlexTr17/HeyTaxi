...
public class PoliceCarAiControl : MonoBehaviour
{
        ...
        public float frontSideSensorsPosition = 0.2f;
        public float frontSensorAngle = 30f;
        private void Awake()
        {
            //enemy = new EnemySight();
        ...
        private void FixedUpdate()
        {
            ...
            else if (gameObject.GetComponent<EnemySight>().state == State.suspicion && gameObject.tag == "Police")
            { SetTarget(way_point); }
            else if (gameObject.GetComponent<EnemySight>().state == State.chase && gameObject.tag == "Police")
            ...
            else if (m_Target == Player.transform)
            {

                Vector3 offsetTargetPos = Player.transform.position;
                float accelBrakeSensitivity = m_AccelSensitivity;

                // decide the actual amount of accel/brake input to achieve desired speed.
                float accel = Mathf.Clamp((m_CarController.CurrentSpeed) * accelBrakeSensitivity, -1, 1);
        ...
        private void Sensors()
        {
            ...
            //front right sensor
            sensorStartPos +=transform.right * frontSideSensorsPosition;
            if (Physics.Raycast(sensorStartPos, transform.forward, out hit, sensorLength))
            {
                if (!hit.collider.CompareTag("Obstacle"))
                {
            ...
            //front left sensor
            sensorStartPos -= transform.right * frontSideSensorsPosition*2;
            if (Physics.Raycast(sensorStartPos, transform.forward, out hit, sensorLength))
            {
            ...
            if (avoiding)
            {
                m_CarController.m_WheelColliders[0].steerAngle = m_CarController.m_MaximumSteerAngle * avoidMultiplier;
                m_CarController.m_WheelColliders[1].steerAngle = m_CarController.m_MaximumSteerAngle * avoidMultiplier;
                float desiredSpeed = m_CarController.MaxSpeed;
                float accelBrakeSensitivity = (desiredSpeed < m_CarController.CurrentSpeed)
                                                ? m_BrakeSensitivity
                                                : m_AccelSensitivity;
                float steer = Mathf.Clamp(avoidMultiplier * m_SteerSensitivity, -1, 1) * Mathf.Sign(m_CarController.CurrentSpeed);
                float accel = Mathf.Clamp((m_CarController.CurrentSpeed) * accelBrakeSensitivity, -1, 1);
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            {
                gameObject.GetComponent<EnemySight>().state = State.chase;
            }
        }
